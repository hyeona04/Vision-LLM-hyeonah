...
while True:
    ...
    for hand in result.hand_landmarks:
        ...
        distance_8_0 = np.hypot(
            index_x - wrist_x,
            index_y - wrist_y
        )

        distance_20_0 = np.hypot(
            pinky_x - wrist_x,
            pinky_y - wrist_y
        )
        ...
        # 8-0, 20-0 둘 다 멀면 보자기
        if distance_8_0 > rps_threshold and distance_20_0 > rps_threshold:
            gesture = "PAPER"

        # 8-0, 20-0 둘 다 가까우면 주먹
        elif distance_8_0 < rps_threshold and distance_20_0 < rps_threshold:
            gesture = "ROCK"

        # 8-0은 멀고, 20-0은 가까우면 가위
        elif distance_8_0 > rps_threshold and distance_20_0 < rps_threshold:
            gesture = "SCISSORS"


        for tip_idx, note in touch_map.items():
